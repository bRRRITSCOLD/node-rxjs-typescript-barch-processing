...
//models
import { BatchProcessingOptions, defaultBatchProcessingOptions, validateBatchProcessingOptions } from '../models';

// data-management
import { retrieveCompanies, retrieveCompanyOrders, sendBulkEmails } from '../data-management';

export const approach4ObservableImproved = async (options: BatchProcessingOptions = {}) => {
  options = {
  ...
  return controller$
    .pipe(
       ...
       */
      mergeMap(curOffset => retrieveCompanies(options.batchSize as any, curOffset), undefined, options.retrieveCompaniesConcurrency),
      /**
       ...
       */
      bufferTime(500, undefined, options.maxBulkEmailCount as any),
      /**
      ...
      scan(
        (acc: any, companies) => {
          ...
          while (queueSize + (options.batchSize as any) <= (options.maxQueueSize as any)) {
            queueSize += options.batchSize as any;
